...
public class Liste extends Liste_abstract{
...
//    }
    public Liste() {
        this.tabEtudiant = new Etudiant[LMAX];
        this.longueur_liste = 0;
    }
    ...
    @Override
    public Liste inserer(Liste L, int p, Etudiant e) {
        int j ;
        if(L.longueur_liste < LMAX){
            if(L.longueur_liste != 0 && p != L.longueur_liste + 1){
                for(j = L.longueur_liste; j >= p; j--){
                    L.tabEtudiant[j+1] = L.tabEtudiant[j];
                }
            }
            L.tabEtudiant[p] = e;
            L.longueur_liste = L.longueur_liste + 1;
